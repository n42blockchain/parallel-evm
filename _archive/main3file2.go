...
// 在将header,block,地址,snap等统一打包,存储到文件中(一个区块的执行就依赖这些东西,此外合约直接从mdbx里边读取),
var (
...
func openKV(label kv.Label, logger log.Logger, path string, exclusive bool) kv.RwDB {
	opts := kv2.NewMDBX(logger).Freezer(snapDir).NoBaseFreezerReadable(true).NoSnapFreezerReadable(true).Path(path).Label(label)
	if exclusive {
		opts = opts.Exclusive()
	}
...
func main() {
	...
	case *kv2.MdbxKV:
		...
		stage.SpawnGenEntireFilesStage(i, stopBlockNumber, ctx, cfg)
	default:
		return

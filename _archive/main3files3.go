...
// 在将header,block,地址,snap等统一打包后(一个区块的执行就依赖这些东西,此外合约直接从mdbx里边读取),
// 在当前累中读取该打包,直接运行,就再去单独读取header等.
var (
...
func openKV(label kv.Label, logger log.Logger, path string, exclusive bool) kv.RwDB {
	opts := kv2.NewMDBX(logger).Freezer(snapDir).NoBaseFreezerReadable(true).Path(path).Label(label)
	if exclusive {
		opts = opts.Exclusive()
	}
...
func main() {
	...
	stage.SpawnVerifysEntireStage(fromBlockNumber, stopBlockNumber, ctx, cfg)
}

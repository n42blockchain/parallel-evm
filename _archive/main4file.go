...
// 串行验证快照是否准确,以及测试使用快照时的速度,支持文件存储快照的方式.
// 使用snapdir下边来读取相应的文件和快照,注意,这时读取区块'header等直接从文件中读取.
var (
	datadir                        = "/Volumes/1T/erigon"
	fromBlockNumber                = uint64(0)
	stopBlockNumber                = uint64(6000000)
...
func openKV(label kv.Label, logger log.Logger, path string, exclusive bool) kv.RwDB {
	opts := kv2.NewMDBX(logger).Freezer(snapDir).Path(path).Label(label)
	if exclusive {
		opts = opts.Exclusive()
	}
...
func main() {
	...
	stage.SpawnVerifyBlocksStage(fromBlockNumber, stopBlockNumber, ctx, cfg)
}

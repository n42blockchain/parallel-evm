...
import (
	...
	_ "starlink-world/erigon-evm/consensus/aura"
	_ "starlink-world/erigon-evm/consensus/bor"
	"starlink-world/erigon-evm/consensus/ethash"
	_ "starlink-world/erigon-evm/consensus/parlia"
	_ "starlink-world/erigon-evm/consensus/serenity"
...
// 并发验证快照是否准确,以及测试使用快照时的速度.另外有单独的线程负责准备相应的数据.
// 为了支持性能对比,还可以选择不使用快照来对比执行速度.
// 如果使用快照,那么就会使用snapdir下边来读取相应的文件和快照,注意,这时读取区块'header等直接从文件中读取.
var (
...
func openKV(label kv.Label, logger log.Logger, path string, exclusive bool) kv.RwDB {
	...
	opts := kv2.NewMDBX(logger).RoTxsLimiter(ch).Path(path).Label(label)
	//todo 如果使用快照,那么就会使用snapdir下边来读取相应的文件和快照,注意,这时读取区块'header等直接从文件中读取.
	if useSnap && len(snapDir) > 0 {
		opts = opts.Freezer(snapDir)
	}
	if exclusive {
		opts = opts.Exclusive()
	}
...
func main() {
	...
	stage.SpawnVerifysBlock2Stage(fromBlockNumber, stopBlockNumber, useSnap, ctx, cfg)
}

...
import (
	...
	"starlink-world/erigon-evm/consensus/misc"
	...
	"starlink-world/erigon-evm/core/vm/evmtypes"
...
func NewWorker(lock sync.Locker, ctx context.Context, background bool, chainDb kv.RoDB, rs *state.StateV3, in *task.QueueWithRetry, blockReader interfaces.FullBlockReader, chainConfig *chain.Config, genesis *types.Genesis, results *task.ResultsQueue, engine consensus.Engine) *Worker {
	w := &Worker{
		lock:        lock,
		chainDb:     chainDb,
		in:          in,
...
func (rw *Worker) RunTxTask(txTask *task.TxTask) {
	rw.lock.Lock()
	defer rw.lock.Unlock()
	rw.RunTxTaskNoLock(txTask)
}

func (rw *Worker) RunTxTaskNoLock(txTask *task.TxTask) {
	if rw.background && rw.chainTx == nil {
		var err error
	...
	rw.stateWriter.RestWS(txTask.BlockNum)
	defer state.ReturnChangeSetWriterPlainV3(rw.stateWriter.ChangeSetWriter())
...
func NewWorkersPool(lock sync.Locker, ctx context.Context, background bool, chainDb kv.RoDB, rs *state.StateV3, in *task.QueueWithRetry, blockReader interfaces.FullBlockReader, chainConfig *chain.Config, genesis *types.Genesis, engine consensus.Engine, workerCount int) (reconWorkers []*Worker, applyWorker *Worker, rws *task.ResultsQueue, clear func(), wait func()) {
	reconWorkers = make([]*Worker, workerCount)

	resultChSize := workerCount * 8
	rws = task.NewResultsQueue(resultChSize, workerCount) // workerCount * 4
	{

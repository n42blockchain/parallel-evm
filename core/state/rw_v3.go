...
func NewStateV3(tmpdir string) *StateV3 {
	rs := &StateV3{
		...
		AccChangeSets: map[uint64]map[string][]byte{},
		StChangeSets:  map[uint64]map[string][]byte{},
	}
	return rs
}
...
func (rs *StateV3) Flush(ctx context.Context, rwTx kv.RwTx, logPrefix string, logEvery *time.Ticker) error {
	...
	if nil != rs.AccChangeSets {
		bns := make([]uint64, 0, len(rs.AccChangeSets))
		for k, _ := range rs.AccChangeSets {
			bns = append(bns, k)
		}
		...
		for _, num := range bns {
			mChanges := rs.AccChangeSets[num]
			if len(mChanges) == 0 {
				continue
			}
			changes, err := RecoverChangeSet(mChanges, false)
			if nil != err {
				return err
			}
			if err := historyv2.Mapper[kv.AccountChangeSet].Encode(num, changes, func(k, v []byte) error {
	...
	if nil != rs.StChangeSets {
		bns := make([]uint64, 0, len(rs.StChangeSets))
		for k, _ := range rs.StChangeSets {
			bns = append(bns, k)
		}
		sort.Slice(bns, func(i, j int) bool {
			return bns[i] < bns[j]
		...
		for _, num := range bns {
			mChanges := rs.StChangeSets[num]
			if len(mChanges) == 0 {
				continue
			}
			changes, err := RecoverChangeSet(mChanges, true)
			if nil != err {
				return err
			}
			if err := historyv2.Mapper[kv.StorageChangeSet].Encode(num, changes, func(k, v []byte) error {
...
func (rs *StateV3) writeStateHistory(roTx kv.Tx, txTask *task.TxTask, agg *libstate.AggregatorV3) error {
	...
	// add prev account storage to Agg
...
func (rs *StateV3) applyState(roTx kv.Tx, txTask *task.TxTask, agg *libstate.AggregatorV3) error {
	...
	if txTask.AcChangeSet != nil && len(txTask.AcChangeSet.Changes) > 0 {
		if cs, ok := rs.AccChangeSets[txTask.BlockNum]; !ok {
			mp := make(map[string][]byte, txTask.AcChangeSet.Len())
			for _, obj := range txTask.AcChangeSet.Changes {
				mp[string(obj.Key)] = obj.Value
			}
			rs.AccChangeSets[txTask.BlockNum] = mp
		} else {
			for _, obj := range txTask.AcChangeSet.Changes {
				if _, ok := cs[string(obj.Key)]; !ok {
					cs[string(obj.Key)] = obj.Value
				}
			}
		}
	}

	if txTask.StChangeSet != nil && len(txTask.StChangeSet.Changes) > 0 {
		if cs, ok := rs.StChangeSets[txTask.BlockNum]; !ok {
			mp := make(map[string][]byte, txTask.StChangeSet.Len())
			for _, obj := range txTask.StChangeSet.Changes {
				mp[string(obj.Key)] = obj.Value
			}
			rs.StChangeSets[txTask.BlockNum] = mp
		} else {
			for _, obj := range txTask.StChangeSet.Changes {
				if _, ok := cs[string(obj.Key)]; !ok {
					cs[string(obj.Key)] = obj.Value
...
func (w *StateWriterV3) ChangeSetWriter() *ChangeSetWriterV3 {
	return w.ws
}
...
func (w *StateWriterV3) RestWS(blockNumber uint64) {
	w.ws = NewChangeSetWriterPlainV3(blockNumber)
}

func (w *StateWriterV3) ChangeSet() (*historyv2.ChangeSet, *historyv2.ChangeSet, error) {
	if w.ws == nil {
		return nil, nil, NoChangeSet
	}

	a, err := w.ws.GetAccountChanges()
	if nil != err {
		return nil, nil, fmt.Errorf("getAccountChanges failed")
	}

	s, err := w.ws.GetStorageChanges()
	if nil != err {
		return nil, nil, fmt.Errorf("getStorageChanges failed")
	}
...
func (w *StateWriterV3) UpdateAccountData(address common.Address, original, account *accounts.Account) error {
	if w.ws != nil {
		if err := w.ws.UpdateAccountData(address, original, account); err != nil {
			return err
		}
	}
...
func (w *StateWriterV3) UpdateAccountCode(address common.Address, incarnation uint64, codeHash common.Hash, code []byte) error {
	if w.ws != nil {
		if err := w.ws.UpdateAccountCode(address, incarnation, codeHash, code); err != nil {
			return err
		}
	}
...
func (w *StateWriterV3) DeleteAccount(address common.Address, original *accounts.Account) error {
	if w.ws != nil {
		if err := w.ws.DeleteAccount(address, original); err != nil {
			return err
		}
	}
...
func (w *StateWriterV3) WriteAccountStorage(address common.Address, incarnation uint64, key *common.Hash, original, value *uint256.Int) error {
	if w.ws != nil {
		if err := w.ws.WriteAccountStorage(address, incarnation, key, original, value); err != nil {
			return err
		}
	}
...
func (w *StateWriterV3) CreateContract(address common.Address) error {
	if w.ws != nil {
		if err := w.ws.CreateContract(address); err != nil {
			return err
		}
	}
	return nil
